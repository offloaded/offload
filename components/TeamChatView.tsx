...
export function TeamChatView({
  ...
  const send = useCallback(() => {
    ...
    sendTeam(CHAT_ID, teamId, text, conversationIdRef.current, mentions);
    setInput("");
    setMentionOpen(false);
    if (inputRef.current) inputRef.current.style.height = "auto";
  }, [input, streaming, teamAgents, CHAT_ID, teamId]);
  ...
  const renderMessage = (msg: ChatMessage, idx: number) => {
    ...
    return (
      <div key={msg.id || idx} className="px-4 py-2 md:px-6">
  ...
  return (
    <div className="flex-1 flex flex-col bg-[var(--color-surface)] overflow-hidden">
      ...
      {/* Input */}
      <div className="fixed bottom-0 left-0 right-0 z-50 md:static md:z-10 md:shrink-0 bg-[var(--color-surface)] px-3 pt-2 pb-[max(16px,env(safe-area-inset-bottom))] md:px-5 md:pb-5">
